 ...
 */
function buildFakeSupabase(overrides: {
...
function buildFakeAuth(userId: string | null = 'user-uuid') {
    return {
        user: vi.fn().mockReturnValue(
            userId ? { id: userId, email: 'test@example.com' } : null,
        ),
    };
}
...
function setup(
    ...
    return { service, fakeSupabase, fakeAuth };
}
