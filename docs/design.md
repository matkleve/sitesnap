...
9. **Marker overlap displacement and dynamic pointer tails:** when two or more square markers overlap in screen space, markers should separate so all squares are visible while tails still point to true coordinates. Defer to post-MVP if needed; MVP must keep square body + pointer shape and avoid hidden marker bodies.# GeoSite – Product Design
